...
def test_get_all_students_order():
    student1 = Student(id=1, name="John", age=17, class_="year 12")
    student2 = Student(id=2, name="Jane", age=18, class_="year 13")
    client.post("/students/1", json=student1.model_dump())
    client.post("/students/2", json=student2.model_dump())
    response = client.get("/students")
    assert response.status_code == 200
